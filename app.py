...
# ==============================================================================
# 分頁 1: 賣出成交圖自動掃描與多組獨立再投資分配 (支援 6 檔買入目標)
# ==============================================================================
with tab1:
    ...
    with st.expander("➕ 手動新增或修正賣出股票資料庫", expanded=not bool(st.session_state["scanned_items"])):
        ...
        for i in range(1, 11):
            ...
            with target_col:
                ...
                with sm1:
                    m_code = st.text_input(f"手動股票 #{i} 代號", placeholder="例如：2376", key=f"m_code_v18_{i}")
                with sm2:
                    m_amt = st.number_input(f"手動股票 #{i} 成交價金", min_value=0.0, value=0.0, step=1000.0, key=f"m_amt_v18_{i}")
    ...
    for g in range(1, st.session_state["group_count"] + 1):
            ...
            for idx, item in enumerate(st.session_state["scanned_items"]):
                ...
                with c_target:
                    use_item = st.checkbox(
                        ...
                        value=(g == 1),
                        key=f"g{g}_chk_v18_{idx}"
                    )
                    if use_item:
                        g_sold_total += item["amount"]
        else:
            st.info("請先上傳截圖或於上方手動輸入賣出股票資訊。")

        st.metric(f"組別 #{g} 賣出金額小計", f"NT$ {g_sold_total:,.0f}")

        # 設定再投資比例
        g_pct = st.slider(f"組別 #{g} 投入再投資比例 (%)", min_value=1, max_value=100, value=100, key=f"g{g}_pct_v18")
        g_reinvest_budget = g_sold_total * (g_pct / 100.0)
        st.info(f"組別 #{g} 實際可用再投資預算（{g_pct}%）：**NT$ {g_reinvest_budget:,.0f}**")

        # 設定買入目標（最多 6 檔）
        st.write(f"**組別 #{g}：設定預計買入股票（最多 6 檔）**")
        g_buy_targets = []
        g_total_weight = 0.0
        ...
        for b in range(1, 7):
            ...
            with b1:
                b_code = st.text_input(f"組別#{g} 買入#{b} 代號", placeholder="例如：2330", key=f"g{g}_b_code_v18_{b}")
            with b2:
                b_price = st.number_input(f"組別#{g} 買入#{b} 掛單單價", min_value=0.0, value=0.0, step=0.5, key=f"g{g}_b_price_v18_{b}")
            with b3:
                b_weight = st.number_input(f"組別#{g} 買入#{b} 分配比例 (%)", min_value=0.0, max_value=100.0, value=0.0, step=5.0, key=f"g{g}_b_weight_v18_{b}")
    ...
    # 3. 執行全案總算
    if st.button("計算所有組別委託買入股數", type="primary", key="btn_calc_all_groups_v18"):
                ...
                if len(grp["targets"]) == 0:
                    ...
                    for idx, target in enumerate(grp["targets"]):
                        ...
                        with col_target:
                            ...
                            st.write(f"**掛單單價:** NT$ {target['price']:,.2f}")
...
with tab2:
    ...
    with col_r2:
        ratio_nainai = st.number_input("奶奶 倍率", min_value=0.0, max_value=5.0, value=0.266, step=0.001, format="%.4f", key="t2_r_nainai_v18")
    with col_r3:
        ratio_yipo = st.number_input("姨婆 倍率", min_value=0.0, max_value=5.0, value=0.150, step=0.001, format="%.4f", key="t2_r_yipo_v18")
    with col_r4:
        ratio_baba = st.number_input("爸爸 倍率", min_value=0.0, max_value=5.0, value=0.200, step=0.001, format="%.4f", key="t2_r_baba_v18")
    with col_r5:
        ratio_lina = st.number_input("Lina 倍率", min_value=0.0, max_value=5.0, value=0.100, step=0.001, format="%.4f", key="t2_r_lina_v18")
    ...
    for i in range(1, 11):
        ...
        with c1:
            stk_code = st.text_input(f"股票 #{i} 代號/名稱", placeholder="例如：2330", key=f"t2_code_v18_{i}")
        with c2:
            stk_price = st.number_input(f"股票 #{i} 掛單單價 (NTD)", min_value=0.0, value=0.0, step=0.5, key=f"t2_price_v18_{i}")
        with c3:
            stk_budget = st.number_input(f"股票 #{i} 舅舅預算 (NTD)", min_value=0.0, value=0.0, step=1000.0, key=f"t2_budget_v18_{i}")
    ...
    # 3. 執行試算與多帳戶結果顯示
    if st.button("計算全帳戶委託股數", type="primary", key="btn_calc_t2_v18"):
